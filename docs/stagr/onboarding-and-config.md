...
both; a minimal or Codex-free profile needs no App), the **Codex App** must be installed once at
...
This setting is required only when the Codex review stage includes `pr_opened` in its `triggers`
(the default profile does); stages configured with only `pr_updated` or `manual` triggers do not
rely on the App’s open trigger.
