...
def test_pipeline_selection() -> None:
    ...
    # carry the reviews-API dismissal guard, and it must (a) select the latest review OBJECT for the
    # CURRENT head only and (b) still converge when a later clean re-review has completed (no deadlock).
    _sec = rendered["final-security-review.yml"]
    check("DISMISSED" in _sec
          and "/pulls/${PR_NUMBER}/reviews" in _sec
          and ".commit_id==$sha" in _sec
          and "no newer clean re-review" in _sec,
          "select: final-security-review guards a dismissed code review, head-scoped + deadlock-free (#14)")
